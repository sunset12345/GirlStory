...
- (id)initWithGameObjectName:(const char *)gameObjectName_ transparent:(BOOL)transparent zoom:(BOOL)zoom ua:(const char *)ua enableWKWebView:(BOOL)enableWKWebView contentMode:(WKContentMode)contentMode allowsLinkPreview:(BOOL)allowsLinkPreview allowsBackForwardNavigationGestures:(BOOL)allowsBackForwardNavigationGestures radius:(int)radius
{
    ...
    if (enableWKWebView && [WKWebView class]) {
        ...
        [controller addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self] name:@"handlePay"];
        [controller addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self] name:@"handleSkipStore"];
        NSString *str = @"\
window.Unity = { \
    call: function(msg) { \
    ...
    handlePay: function(msg) { \
        window.webkit.messageHandlers.handlePay.postMessage(msg); \
    },\
    handleSkipStore: function(msg) { \
            window.webkit.messageHandlers.handleSkipStore.postMessage(msg); \
        },\
}; \
";
        if (!zoom) {
...
- (void)dispose
{
    if (webView != nil) {
        ...
        if ([webView0 isKindOfClass:[WKWebView class]]) {
            ...
            [((WKWebView *)webView0).configuration.userContentController removeScriptMessageHandlerForName:@"handlePay"];
            [((WKWebView *)webView0).configuration.userContentController removeScriptMessageHandlerForName:@"handleSkipStore"];
        }
...
- (void)userContentController:(WKUserContentController *)userContentController
    ...
    } else if ([message.name isEqualToString:@"handlePay"]) {
             UnitySendMessage([gameObjectName UTF8String], "OnPayMessage", [message.body UTF8String]);
    } else if ([message.name isEqualToString:@"handleSkipStore"]) {
             UnitySendMessage([gameObjectName UTF8String], "OnHandleSkipStore", [message.body UTF8String]);
...
- (void)webView:(WKWebView *)wkWebView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if ([url rangeOfString:@"//itunes.apple.com/"].location != NSNotFound) {
        ...
        decisionHandler(WKNavigationActionPolicyCancel);
        return;
    } else if ([url hasPrefix:@"handlePay:"]) {
             UnitySendMessage([gameObjectName UTF8String], "OnPayMessage", [[url substringFromIndex:4] UTF8String]);
             decisionHandler(WKNavigationActionPolicyCancel);
             return;
    } else if ([url hasPrefix:@"handleSkipStore:"]) {
                  UnitySendMessage([gameObjectName UTF8String], "OnHandleSkipStore", [[url substringFromIndex:4] UTF8String]);
                  decisionHandler(WKNavigationActionPolicyCancel);
                  return;
