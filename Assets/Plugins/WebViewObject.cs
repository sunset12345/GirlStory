...
public class WebViewObject : MonoBehaviour
{
        ...
        bool separated = false)
    {
        ...
        onCookies = cookies;
        onPay = pay;
        onHandleSkipStore = handleSkipStore;
#if UNITY_WEBGL
#if !UNITY_EDITOR
    ...
    public void OnPayMessage(string message)
    {
        onPay?.Invoke(message);
    }
    
    public void OnHandleSkipStore(string message)
    {
        onHandleSkipStore?.Invoke(message);
    }
    ...
    void Update()
    {
        ...
        for (;;) {
            ...
            Debug.Log($"Message {s.Substring(0, i)}");
            switch (s.Substring(0, i)) {
            case "CallFromJS":
                CallFromJS(s.Substring(i + 1));
                break;
            case "OnPayMessage":
                OnPayMessage(s.Substring(i + 1));
                break;
            case "OnHandleSkipStore":
                OnHandleSkipStore(s.Substring(i + 1));
                break;
            case "CallOnError":
                CallOnError(s.Substring(i + 1));
                break;
